...
```javascript
    ...
    var obs = ko.observable().extend({crud: [model, 'record123']);

    // fetch the user's id (record123)
    obs.userId();
...
```javascript
    ...
    var list = ko.observableArray().extend({crud: [model, {end: Store.LAST, limit: 10}]});
...
```javascript
    ...
    var view = new View();
    model.sync(view).crud.read('record123');
...
```javascript
   ...
   var view = {
      list: ko.observableArray().extend({crud: model});
   }

   // syncs a single record to an observable
   var rec = ko.observable().extend({crud: model});

   // you can also generate crud variables by calling model.sync()
   model.sync(observableArray);
   model.sync(view);
   model.sync(observable);
```

### Perform manual CRUD operations on a view

Once an object is synced, it has a `crud` member which can be used outside knockout bindings to manually perform
operations:
...
```javascript
    ...
    // delete the record from database, we are no longer synced to the record
    view.crud.delete();
...
### Perform manual CRUD operations on observable arrays
...
```javascript
    // sync an array with the server and load the entire database table
    var users = ko.observableArray().extend({crud: model});

    // load some users from the database (users created in the last hour, up to 100 records)
    // we are now synchronized to this filtered list of records and any previous data is deleted
    users.crud.read( {where: {created: {greaterThan: new Date().valueOf() - 3600}}, limit: 100} );

    // a much faster search can be performed by storing the user records with a sort field based on the timestamp
    // then start/end/limit may be used:
    users.crud.read( {start: new Date().valueOf() - 3600, limit: 100} );
    ...
    // save updates when model.auto is false
    users.crud.update();

    // or both at the same time
    users.crud.update('update', {id: 'recordXYZ', name: 'Alf'});
...
# Missing Stuff (TODO)
 ...
 - sorted records aren't perfectly synchronized on the knockout side; need to use a comparator for these instead of prevId
 - concurrent edits are not merged (yet); if client changes data and server changes same record, the later change wins
 - composite keys are not thoroughly tested and surely have some minor bugs to address
 - validation isn't implemented yet (soon, soon)
 - Undo operations
 - Offline storage modes
 - Add a chain() method to Crud/CrudArray that only continues if previous operation succeeds (right now it always continues)
 - Need a generic store test unit that can be used for any store implementation
...
The model represents a database table (a bucket, path, etc). It can be synchronized to a single Record or to a list of Records.
...
This is equivalent to calling ko.observable(...).extend({crud: model});

This method applies all the Model's fields to the `target` object (or to each element if `target` is an observable array),
mapping observable fields as appropriate. It also adds the special [crud](#crud) variable.

This WILL NOT magically start applying updates to the database, unless a record id or some criteria (for arrays)
is passed into the call. This happens because `target` hasn't been linked to any particular record/list yet.

To activate a record after calling sync it is necessary to call `crud.create()` or `crud.read()` in order to hook
up the record(s) to the database.

Hooking up lists is particularly tricky. You must either pass criteria into the `sync` method or call `Crud.read()`
to synchronize to some filtered list on the server. Otherwise, how will we know what we are comparing the local data
 against to decide if anything has been added/removed/changed?
...
The .crud object is added to any observable after calling `Model.sync()` or `ko.observable().extend({crud: ...})`.
It provides a complete set of CRUD methods for syncing data with the data store.

```javascript
   // create an empty, unsynched observable using extend()
   var view = ko.observable().extend({crud: model});

   // synchronize a knockout observable and load a record from the database (it is synchronized as soon as it loads)
   var view = ko.observable().extend({crud: [model, 'recordXYZ']});

   // create a new empty observable using sync()
   var view = {};
   model.sync(view);

   // create a new observable and load a database record using sync()
   var view = {};
   model.sync(view, 'recordXYZ');

   // do it all in one step
   var view = model.sync({}, 'recordXYZ');
```

Crud methods also exist on arrays created with `Model.sync(observableArray)` or `ko.observableArray().extend({crud: ...})`;
Because the arguments and usage are slightly different for arrays, the API is explained below under [Crud.Array](#CrudArray)
...
Save the local record to the database as a new record. This takes place immediately (even if auto-sync is off)
and the record is now synchronized with the server. (this is different than CrudArray's behavior, see below)
...
```javascript
   view.crud.create({name: 'John', email: 'john@here.com'})
      .promise()
...
Update (save) the synchronized record to the database. The save only occurs if changes occur, so it's safe to
call this any time without worrying about superfluous traffic to the server (it does take some overhead to determine
if changes occurred, of course)

```javascript
   function logUpdate(id) { console.log( id===false? 'not saved' : 'saved' ); }

   view.crud.update(); // doesn't do anything (wasn't changed)

   view.data.favoriteColor('green');
   view.crud.update(); // saved to database
```

It is possible to modify the data and save it at the same time by passing a hash of field/values to be saved.

```javascript
   view.crud.read('record123').update( {favoriteColor: 'green'} );
```
...
Deletes the record from the database immediately, without need for an update() call. The behavior for CrudArray
is different (see below)

```javascript
   view.crud.read('record123').delete();
```
...
answer. The promise is never rejected, it simply waits for success or failure.
...
```javascript
   // change the name, save the updates, then return the view object
   view.crud.read('recordId').parent.name('John').crud.update().parent;

   view.crud.parent.crud.parent.crud.parent; // recursive fun
```

<a name="CrudArray" id="crud"></a>
## Crud.Array (applied when `Model.sync()` is invoked on observable array)

A special .crud object is attached to observableArrays after calling `Model.sync()` or
`ko.observableArray().extend({crud: ...})` on any observableArray.
...
```javascript
   // create a list using extend() and load the entire database table into memory
   var list = ko.observableArray().extend({crud: model});
...
The `create()` and `delete()` methods do not automatically send data to the server if auto-update is off. Instead, they
can be used to apply a list of changes, which are then pushed to the server using `update()`
...
Create a new rocerd in the list. For unsorted lists, it is appended to the end. For sorted records, it is spliced into
the list at the appropriate point, based on sort order. This method will accept multiple records or an array of records.

A new record should not have an ID (one should be created by the Store).

When auto-update is off, this DOES NOT immediately send the change to the server; it waits for the next call to
`update()` instead.
...
If any data already exists in the list, it is deleted before adding the read results.
...
### Crud.Array.update( [changes] )

@param changes (see below)
@returns {Promise} resolved after the data store returns results (when auto-update is off)

Save all changes to the database and return the number of records updated. If no records are changed this will simply
return 0 immediately without a call to the server. If auto-updates are on, this will return after the current
sync call (if any is in progress) is completed.

```javascript
   list.crud.update();
   list.crud.promise().then(function(crud) { ... };
```
...
Removes a record (or records) from the list by their IDs.

If auto-update is off, this DOES NOT immediately send the data to the server; it waits for the next `update()` operation.
...
```javascript
    ...
    var list = ko.observableArray().extend({crud: model}).crud.read('recordId').parent.name('John').crud.update().parent;
