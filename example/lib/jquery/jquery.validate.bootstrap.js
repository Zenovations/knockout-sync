// modified from code at https://github.com/tonycoco/rails_template/blob/master/files/assets/javascripts/jquery.validate.bootstrap.js
// (made it work better with IE versions, better default behaviors for non-standard structures)
...
(function($) {
   ...
   $.validator.setDefaults({
      ...
      unhighlight: function(element, errorClass, validClass) {
         var $obj;
         $obj = element.type === 'radio' ? this.findByName(element.name) : $(element);
         $obj.next('span.help-block.' + errorClass).remove();
   ...
   $.validator.prototype.showLabel = function(element, message) {
      ...
      if (label.length) {
         ...
         label = $('<' + this.settings.errorElement + '/>').attr({
            'for': this.idOrName(element),
            'generated': true
         }).addClass(this.settings.errorClass).html(message || '').addClass('help-block');
